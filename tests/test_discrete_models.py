...
def test_SIpRpS():
    ...
    assert len(modelsiprps.traces['time']) == tsteps
...
def test_SEIpRpS():
    ...
    assert len(modelseiprps.traces['time']) == tsteps
...
def test_SIpR():
    modelsipr = SIpR()
    modelsipr([1000, 1, 0], 50, 1001, {'beta': 2, 'gamma': 1, 'b': 0, 'r': .5, 'p': .5})
...
def test_SEIpR():
    ...
    assert len(modelseipr.traces['time']) == tsteps
...
def test_SIRS():
    modelsirs = SIRS()
    modelsirs([1000, 1, 0], 50, 1001, {'beta': 2, 'b': 0, 'r': .5, 'w': .5})
